...
void Eratosthenes4::generate(const int64 x) {
  generate(0, x);
}

void Eratosthenes4::generate(const int64 from, const int64 to) {
  flags_.clear();
  num_primes_ = 0;
  if (from <= 2 && 2 < to)
    ++num_primes_;
...
void Eratosthenes4::initFlags(const int64 from_rem, const int64 to,
...
void Eratosthenes4::generateSmall(const uint64 offset, const int64 sqrt_to) {
  sflags_.clear();
  indecies_.clear();
